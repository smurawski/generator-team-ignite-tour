    ...
    async.series([
        ...
        function (mainSeries) {
            ...
            if (args.type === 'powershell') {
                ...
                findOrCreateBuild(args.tfs, teamProject, token, queues, dockerEndpoint, dockerRegistryEndpoint, args.dockerRegistryId, args.buildJson, args.target, gen, azureEndpoint, mainSeries);
            } else {
                ...
                findOrCreateBuild(args.tfs, teamProject, token, queue.id, dockerEndpoint, dockerRegistryEndpoint, args.dockerRegistryId, args.buildJson, args.target, gen, azureEndpoint, mainSeries);
            }
        }
    ],
...
function findOrCreateBuild(account, teamProject, token, queue,
    ...
    filename, target, gen, azureEndpoint, callback) {
    'use strict';

    util.tryFindBuild(account, teamProject, token, gen.type, target, function (e, bld) {
        if (e) {
            callback(e);
        }
...
function createBuild(account, teamProject, token, queues,
    ...
    var tokens = {
        ...
        '{{ServiceConnectionId}}': azureEndpoint ? azureEndpoint.id : null
    };
