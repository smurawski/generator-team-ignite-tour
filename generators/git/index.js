...
module.exports = class extends Generator {
    ...
    writing() {
        if (this.action === `clone` || this.action === `all`) {
            ...
            // and commit.
            this.log.ok(`Cloning repository ${util.getFullURL(this.tfs)}/${this.applicationName}/_git/${this.repoName}`);

            // By adding the PAT right after https:// I can clone a repository without 
            // asking user for credentials 
            let url = `${util.getFullURL(this.tfs)}/${this.applicationName}/_git/${this.repoName}`;
    ...
    // 8. Called last, cleanup, say good bye, etc
        ...
        if (this.action === `commit` || this.action === `all`) {
            process.chdir(path.join(this.destinationRoot(), this.repoName));
        ...
        if (this.action === `push`) {
            this.log.ok(`pushing ${this.repoName} to Azure DevOps.`);
            this.spawnCommandSync(`git`, [`push`, `origin`, `master`], {
                stdio: ['pipe', 'pipe', process.stderr]
            });
            this.log.ok(`origin/master is now up to date.`);
        }
    }
};